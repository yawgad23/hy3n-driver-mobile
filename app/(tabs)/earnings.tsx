...
export default function DriverEarningsScreen() {
  ...
  const totals = overview?.totals || { gross: 0, net: 0, tips: 0, dailyPlatformFee: 0, dailyFeeDays: 0, tripCount: 0, averagePerTrip: 0, availableBalance: 0 };
  const dailyFee = overview?.dailyFee || { amount: 50, status: 'unpaid', date: '' };
  ...
  const feeStatus = String(dailyFee.status).toLowerCase();
  const feeStatusLabel = feeStatus === 'paid' ? 'Paid today' : feeStatus === 'processing' ? 'Payment pending' : 'Not paid today';
  const feeStatusColor = feeStatus === 'paid' ? GREEN : feeStatus === 'processing' ? GOLD : '#F87171';

  return <View style={[styles.container, dynamic.container]}>
    <View style={[styles.header, { paddingTop: insets.top + 10, borderBottomColor: themeColors.border }]}><Text style={[styles.headerTitle, dynamic.text]}>Earnings</Text><Text style={styles.headerSub}>You keep 100% of your fares and tips</Text></View>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
      ...
      {financeQuery.isLoading ? <ActivityIndicator color={GOLD} style={{ marginVertical: 46 }} /> : financeQuery.isError ? <View style={[styles.loadError, dynamic.card]}><MaterialIcons name="cloud-off" size={30} color={GOLD} /><Text style={[styles.cardTitle, dynamic.text]}>Earnings could not be loaded</Text><Text style={[styles.emptyText, dynamic.muted]}>Your completed trips are safe. Check your connection and try again.</Text><TouchableOpacity style={styles.retryButton} onPress={() => financeQuery.refetch()}><Text style={styles.retryButtonText}>Retry</Text></TouchableOpacity></View> : <>
        <View style={[styles.heroCard, dynamic.card]}><Text style={[styles.heroLabel, dynamic.muted]}>YOUR EARNINGS · {title.toUpperCase()}</Text><Text style={styles.heroAmount}>GH₵{Number(totals.net).toFixed(2)}</Text><View style={styles.heroMeta}><Text style={styles.heroMetaText}>{totals.tripCount} completed trips</Text><Text style={styles.heroMetaText}>Avg. GH₵{Number(totals.averagePerTrip).toFixed(2)} / trip</Text></View></View>
        <View style={styles.kpiGrid}><Kpi icon="payments" label="Gross fares" value={totals.gross} color={GOLD} dynamic={dynamic} /><Kpi icon="account-balance-wallet" label="Tips" value={totals.tips} color={GREEN} dynamic={dynamic} /><Kpi icon="event-available" label="Fee payments" value={totals.dailyPlatformFee} color={BLUE} dynamic={dynamic} /><Kpi icon="local-taxi" label="Trips" value={totals.tripCount} color={PURPLE} dynamic={dynamic} count /></View>
        <View style={[styles.dailyFeeCard, dynamic.card]}><View style={styles.dailyFeeIcon}><MaterialIcons name="event-repeat" size={22} color={GOLD} /></View><View style={{ flex: 1 }}><Text style={[styles.cardTitle, dynamic.text]}>Daily platform fee</Text><Text style={[styles.cardCaption, dynamic.muted]}>GH₵{Number(dailyFee.amount).toFixed(0)} once per day, paid separately by MoMo</Text></View><View style={styles.feeStatus}><Text style={[styles.feeStatusText, { color: feeStatusColor }]}>{feeStatusLabel}</Text></View></View>
        {period !== 'today' ? <Text style={[styles.periodFeeNote, dynamic.muted]}>GH₵{Number(totals.dailyPlatformFee).toFixed(0)} in confirmed or pending platform-fee payments across this period.</Text> : null}
        <View style={[styles.card, dynamic.card]}><View style={styles.cardTitleRow}><View><Text style={[styles.cardTitle, dynamic.text]}>Earnings trend</Text><Text style={[styles.cardCaption, dynamic.muted]}>Full fares and tips by completed-trip date</Text></View><MaterialIcons name="show-chart" size={23} color={GOLD} /></View>{trend.length === 0 ? <Text style={[styles.emptyText, dynamic.muted]}>Complete trips to see your earnings trend.</Text> : <View style={styles.barChart}>{trend.map((item) => <View key={item.date} style={styles.barColumn}><Text style={[styles.barValue, dynamic.muted]}>{item.amount ? `₵${Math.round(item.amount)}` : ''}</Text><View style={[styles.barTrack, { backgroundColor: isDark ? '#242424' : '#F3F4F6' }]}><View style={[styles.barFill, { height: `${Math.max(5, (item.amount / maxTrend) * 100)}%` }]} /></View><Text style={[styles.barLabel, dynamic.muted]}>{trendLabel(item.date, period)}</Text></View>)}</View>}</View>
        <View style={[styles.card, dynamic.card]}><Text style={[styles.cardTitle, dynamic.text]}>Your earnings breakdown</Text><View style={styles.breakdownRow}><Text style={[styles.breakdownLabel, dynamic.muted]}>Gross fares</Text><Text style={[styles.breakdownValue, dynamic.text]}>GH₵{Number(totals.gross).toFixed(2)}</Text></View><View style={styles.breakdownRow}><Text style={[styles.breakdownLabel, dynamic.muted]}>Tips</Text><Text style={[styles.breakdownValue, { color: GREEN }]}>+ GH₵{Number(totals.tips).toFixed(2)}</Text></View><View style={styles.breakdownRow}><Text style={[styles.breakdownLabel, dynamic.muted]}>Trip commission</Text><Text style={[styles.breakdownValue, { color: GREEN }]}>GH₵0.00</Text></View><Text style={[styles.dailyFeeExplanation, dynamic.muted]}>HY3N does not deduct a percentage from trips. The GH₵50 daily platform fee is charged separately to your MoMo account.</Text><View style={[styles.totalRow, { borderTopColor: themeColors.border }]}><Text style={[styles.totalLabel, dynamic.text]}>Your total earnings</Text><Text style={styles.totalValue}>GH₵{Number(totals.net).toFixed(2)}</Text></View></View>
        <View style={[styles.balanceCard, dynamic.card]}><View><Text style={[styles.cardTitle, dynamic.text]}>Available to withdraw</Text><Text style={[styles.cardCaption, dynamic.muted]}>Full fares and tips, less payout requests</Text></View><Text style={styles.balanceAmount}>GH₵{Number(totals.availableBalance).toFixed(2)}</Text></View>
